...
#ifndef HURCHALLA_UTIL_BRANCHLESS_TWO_TIMES_SHIFT_LEFT_TO_HILO_H_INCLUDED
#define HURCHALLA_UTIL_BRANCHLESS_TWO_TIMES_SHIFT_LEFT_TO_HILO_H_INCLUDED
...
#include "hurchalla/util/detail/platform_specific/impl_branchless_two_times_shift_left_to_hilo.h"
...
T branchless_two_times_shift_left_to_hilo(T& lowResult, T a, int shift)
{
    ...
    T highResult = detail::impl_branchless_two_times_shift_left_to_hilo<T>::call_asm(lowResult, a, shift);
#else
    T highResult = detail::impl_branchless_two_times_shift_left_to_hilo<T>::call(lowResult, a, shift);
#endif

    HPBC_UTIL_POSTCONDITION(lowResult == static_cast<T>((a << shift) << 1));
    HPBC_UTIL_POSTCONDITION(highResult == static_cast<T>(a >> (digitsT - shift - 1)));
    return highResult;
}
