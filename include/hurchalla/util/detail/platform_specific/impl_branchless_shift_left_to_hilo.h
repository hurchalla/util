...
#ifndef HURCHALLA_UTIL_IMPL_BRANCHLESS_SHIFT_LEFT_TO_HILO_H_INCLUDED
#define HURCHALLA_UTIL_IMPL_BRANCHLESS_SHIFT_LEFT_TO_HILO_H_INCLUDED
...
struct impl_branchless_shift_left_to_hilo {
  ...
  HURCHALLA_FORCE_INLINE static T call_asm(T& lowResult, T a, int shift)
  {
    ...
    uint64_t part0 = alo << (ushift32 % 64);
    ...
    uint64_t ahi_right1 = ahi >> 1;
    uint64_t part2 = ahi_right1 >> (ushift_not % 64);
  ...
  HURCHALLA_FORCE_INLINE static T call(T& lowResult, T a, int shift)
  {
    ...
    H ahi_right1 = ahi >> 1;
    H part2 = ahi_right1 >> (ushift_not % HALFBITS);
...
#  if 0
    ...
    static_cast<void>(alo); // silence unused variable warnings
    lowResult = a << ushift;
    unsigned int rshift = static_cast<unsigned int>(digitsT) - ushift;
    return static_cast<T>((a >> (rshift - 1)) >> 1);
#  else
    H part0 = alo << (ushift % HALFBITS);
    H part1 = ahi << (ushift % HALFBITS);
    ...
    H part2 = (ahi >> (ushift_not % HALFBITS)) >> 1;
...
template <typename T>
struct impl_branchless_shift_left_to_hilo<T, typename
  ...
  HURCHALLA_FORCE_INLINE static T call(T& lowResult, T a, int shift)
  {
    ...
    lowResult = static_cast<T>(a << shift);
    int rshift = digitsT - shift;
    HPBC_UTIL_ASSERT2(rshift > 0);
    return static_cast<T>((a >> (rshift - 1)) >> 1);
  }
};
