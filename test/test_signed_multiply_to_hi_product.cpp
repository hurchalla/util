// Copyright (c) 2025 Jeffrey Hurchalla
// --- This file is distributed under the MIT Open Source License, as detailed
// by the file "LICENSE.TXT" in the root of this repository ---


// Strictly for testing purposes, we make sure to enable the inline-asm function
// versions of signed_multiply_to_hi_product.  In their postconditions they
// will call the corresponding non-inline asm version to check their results, so
// we won't miss unit testing of the "normal" function versions too, so long as
// we also enable util's postcondition checking.
#undef HURCHALLA_ALLOW_INLINE_ASM_MULTIPLY_TO_HILO
#define HURCHALLA_ALLOW_INLINE_ASM_MULTIPLY_TO_HILO
#undef HURCHALLA_UTIL_ENABLE_ASSERTS
#define HURCHALLA_UTIL_ENABLE_ASSERTS
#undef HURCHALLA_UTIL_ASSERT_LEVEL
#define HURCHALLA_UTIL_ASSERT_LEVEL 3


#include "hurchalla/util/signed_multiply_to_hi_product.h"
#include "hurchalla/util/traits/ut_numeric_limits.h"
#include "gtest/gtest.h"
#include <cstdint>

namespace {


template <typename T>
void test_signed_multiply_to_hi_product()
{
    namespace hc = ::hurchalla;
    static_assert(hc::ut_numeric_limits<T>::is_integer, "");
    static_assert(hc::ut_numeric_limits<T>::is_signed, "");

    // As with signed_multiply_to_hi_product, this file assumes two's
    // complement and arithmetic right shift for signed integers.

    // Assert two's complement
    static_assert(static_cast<T>(-1) == ~(static_cast<T>(0)), "");
    // Assert arithmetic right shift
    static_assert((static_cast<T>(-1) >> 1) == static_cast<T>(-1), "");

    using U = typename hc::extensible_make_unsigned<T>::type;
    static constexpr int digitsU = hc::ut_numeric_limits<U>::digits;

    T tmax = hc::ut_numeric_limits<T>::max();
    T tmin = hc::ut_numeric_limits<T>::min();
    int udigits = hc::ut_numeric_limits<U>::digits;
    T hi, a, b;

    a = static_cast<T>(1) << (digitsU/2);
    b = a;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 1);
    b = -b;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);
    hi = hc::signed_multiply_to_hi_product(b, a);
    EXPECT_TRUE(hi == -1);
    a = -a;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 1);

    a = static_cast<T>(1) << (digitsU/2);
    b = static_cast<T>(2*a + 1);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 2);
    b = -b;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -3);
    hi = hc::signed_multiply_to_hi_product(b, a);
    EXPECT_TRUE(hi == -3);
    a = -a;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 2);

    a = 5; b = 6;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = 9; b = 7;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = 4; b = 4;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);

    a = 0; b = 0;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = 0; b = 1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = 1; b = 0;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = 0; b = 2;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = tmax; b = 0;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);

    a = 1; b = 1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = 1; b = 2;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = 4; b = 1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = tmax; b = 1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = 1; b = tmax;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);

    a = 2; b = tmax;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = tmax; b = 2;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);

    a = tmax; b = tmax;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == (static_cast<T>(1)<<(udigits-2)) - 1);

    a = tmax; b = static_cast<T>(tmax-1);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == (static_cast<T>(1)<<(udigits-2)) - 2);

    a = static_cast<T>(tmax-2); b = static_cast<T>(tmax-1);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == (static_cast<T>(1)<<(udigits-2)) - 3);

    a = static_cast<T>(tmax-4); b = static_cast<T>(tmax-4);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == (static_cast<T>(1)<<(udigits-2)) - 5);


    // negative number and zero
    a = 0; b = -1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = -1; b = 0;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = 0; b = -2;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = tmin; b = 0;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);


    // both negative numbers
    a = -5; b = -6;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = -9; b = -7;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = -4; b = -4;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);

    a = -1; b = -1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = -1; b = -2;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = -4; b = -1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);

    a = tmin; b = -1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);
    a = -1; b = tmin;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 0);

    a = -2; b = tmin;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 1);
    a = tmin; b = -2;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == 1);

    a = tmin; b = tmin;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == (static_cast<T>(1)<<(udigits-2)));

    a = tmin; b = static_cast<T>(tmin+1);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == (static_cast<T>(1)<<(udigits-2)) - 1);

    a = static_cast<T>(tmin+2); b = static_cast<T>(tmin+1);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == (static_cast<T>(1)<<(udigits-2)) - 2);

    a = static_cast<T>(tmin+4); b = static_cast<T>(tmin+4);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == (static_cast<T>(1)<<(udigits-2)) - 4);


    // one negative and one positive number
    a = -5; b = 6;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);
    a = 9; b = -7;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);
    a = -4; b = 4;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);

    a = -1; b = 1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);
    a = 1; b = -2;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);
    a = 4; b = -1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);

    a = tmin; b = 1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);
    a = 1; b = tmin;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);

    a = tmax; b = -1;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);
    a = -1; b = tmax;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);

    a = 2; b = tmin;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);
    a = tmin; b = 2;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);

    a = -2; b = tmax;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);
    a = tmax; b = -2;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -1);

    a = tmin; b = tmax;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -(static_cast<T>(1)<<(udigits-2)));
    a = tmax; b = tmin;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -(static_cast<T>(1)<<(udigits-2)));

    a = tmax; b = static_cast<T>(tmin+1);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -(static_cast<T>(1)<<(udigits-2)));
    a = static_cast<T>(tmin+1); b = tmax;
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -(static_cast<T>(1)<<(udigits-2)));

    a = static_cast<T>(tmin+2); b = static_cast<T>(tmax-1);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -(static_cast<T>(1)<<(udigits-2)) + 1);
    a = static_cast<T>(tmax-1); b = static_cast<T>(tmin+2);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -(static_cast<T>(1)<<(udigits-2)) + 1);

    a = static_cast<T>(tmin+4); b = static_cast<T>(tmax-4);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -(static_cast<T>(1)<<(udigits-2)) + 4);
    a = static_cast<T>(tmax-4); b = static_cast<T>(tmin+4);
    hi = hc::signed_multiply_to_hi_product(a, b);
    EXPECT_TRUE(hi == -(static_cast<T>(1)<<(udigits-2)) + 4);
}


void test_signed_multiply_exhaustive_int8()
{
    static_assert(-1 == ~0, "");         // the tests assume two's complement
    static_assert((-1 >> 1) == -1, "");  // and arithmetic right shift

    using std::int8_t;
    namespace hc = ::hurchalla;
    int8_t tmax = hc::ut_numeric_limits<int8_t>::max();
    int8_t tmin = hc::ut_numeric_limits<int8_t>::min();

    for (int i=tmin; i<=tmax; i++) {
        for (int j=tmin; j<=tmax; j++) {
            int8_t i8 = static_cast<int8_t>(i);
            int8_t j8 = static_cast<int8_t>(j);
            int8_t hi = hc::signed_multiply_to_hi_product(i8, j8);
            int ref_hi = (i*j) >> 8;
            EXPECT_TRUE(hi == ref_hi);
        }
    }
}


TEST(HurchallaUtil, signed_multiply_to_hi_product) {
    test_signed_multiply_to_hi_product<std::int8_t>();
    test_signed_multiply_to_hi_product<std::int16_t>();
    test_signed_multiply_to_hi_product<std::int32_t>();
    test_signed_multiply_to_hi_product<std::int64_t>();
#if HURCHALLA_COMPILER_HAS_UINT128_T()
    test_signed_multiply_to_hi_product<__int128_t>();
#endif

    test_signed_multiply_exhaustive_int8();
}


} // end unnamed namespace
