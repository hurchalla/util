...
void test_shifts()
{
    ...
    constexpr int bitsLimited = (bitsT < bitsNative) ? bitsT : bitsNative;
    ...
    for (auto val : vec) {
        ...
        for (int i=0; i<bitsLimited; ++i) {
            ...
            EXPECT_TRUE(hc::shift_right_limited(val, i) == (val >> i));
        }
    }
}


TEST(HurchallaUtil, left_and_right_shifts) {
    test_shifts<std::uint8_t>();
    test_shifts<std::uint16_t>();
    test_shifts<std::uint32_t>();
    test_shifts<std::uint64_t>();
#if HURCHALLA_COMPILER_HAS_UINT128_T()
    test_shifts<__uint128_t>();
#endif
}
